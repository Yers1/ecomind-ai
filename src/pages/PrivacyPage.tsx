  ...
  return (
    <div className="privacy-page page-surface">
      ...
      <div className="container privacy-content">
        ...
        <section className="data-choice"><div><h2>Your choices stay in your control</h2><p>The prototype does not sell shopping history. You can analyse as a guest and decide whether to keep local progress.</p><div className="data-choice__list"><span><CheckCircle size={18} weight="fill" /> Analyse as a guest</span><span><CheckCircle size={18} weight="fill" /> Join the leaderboard only if you choose</span><span><CheckCircle size={18} weight="fill" /> Remove wishlist items</span><span><CheckCircle size={18} weight="fill" /> Clear local storage in your browser</span></div></div><div className="local-storage-card"><Cookie size={31} weight="duotone" /><h3>Local prototype storage</h3><p><code>ecomind-ai-demo-state-v2</code></p><small>Stores wishlist IDs, point events, completed actions, preferences and the optional leaderboard nickname. The extension uses its own <code>chrome.storage.local</code> key.</small></div></section>
        <section className="extension-future"><div className="extension-future__icon"><ShieldCheck size={28} /></div><div><h2>Leaderboard participation is optional</h2><p>The local prototype ranks only a nickname, EcoPoints and action counts. It does not publish product names, browsing history, spending, scores or wishlist contents. Leaving removes the local leaderboard profile without deleting private activity or saved products.</p></div></section>
        ...
        <section className="privacy-disclaimer"><h2>Prototype disclaimer</h2><p>EcoMind reads product information from the active page only after you activate it. Analysis is processed locally in this prototype. It is not connected to retailer accounts, payment services, a product-data server or certification bodies, and no official retailer partnership is claimed.</p></section>
