...
import { getTrafficLightStatus, trafficLightAccessibleText } from '../../shared/trafficLight'
...
export function EcoWidget({
  ...
  return (
    <>
      <button className={`eco-widget eco-widget--${state}`} onClick={activate} aria-label={state === 'success' ? `${trafficLightAccessibleText(trafficStatus, result.score, result.provisional, product.confidenceLevel, result.range ? [result.range.min, result.range.max] : null)} Open EcoMind analysis.` : state === 'loading' ? 'EcoMind is analysing this product' : state === 'error' ? 'EcoMind analysis failed. Try again' : 'Activate EcoMind product analysis'}>
