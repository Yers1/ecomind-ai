...
export function KoalaProgress({ points }: { points: number }) {
  ...
  const start = level === 'Starter Koala' ? 0 : level === 'Eco Explorer' ? 15 : 40
    ...
    <section className="koala-progress">
      ...
      <div className="koala-progress__body">
        ...
        <div className="koala-progress__levels">
          {['Starter', 'Explorer', 'Champion'].map((name, index) => <span key={name} className={points >= [0,15,40][index] ? 'is-unlocked' : ''}>{points >= [0,15,40][index] ? <CheckCircle size={15} weight="fill" /> : <LockKey size={14} />}{name}</span>)}
        </div>
      </div>
    </section>
  )
