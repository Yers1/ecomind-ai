        ...
        <div className="score-drawer__content">
          ...
          <section className="drawer-section evidence-section">
            ...
            <div className="certification-summary">
              ...
              {product.certifications.length ? product.certifications.map((item) => <div key={`${item.certificationId}-${item.rawClaim}`}><strong>{item.certificationId === 'oeko-tex-standard-100' ? 'Chemical-safety certification' : item.affectsPeopleInformation ? 'People certification' : item.affectsEnvironmentalScore ? 'Environmental certification' : 'Certification candidate'}: {item.displayedName}</strong><span>{item.status} · {item.rawClaim}{item.certificationIdValue ? ` · label ${item.certificationIdValue}` : ''}</span><small>{item.affectsEnvironmentalScore ? 'Included only after independent product-specific verification' : 'Not included in the environmental Green Score'}</small></div>) : <p>No verified certification found. No points added or removed.</p>}
