...
const factors = [
  ...
  { icon: Package, name: 'Fulfilment packaging', weight: SCORE_WEIGHTS.fulfilmentPackaging, text: 'Packaging added by the retailer or delivery partner, such as a delivery box, mailer, filling or reduced-packaging option.' },
  { icon: Package, name: 'Manufacturer packaging', weight: SCORE_WEIGHTS.manufacturerPackaging, text: 'Packaging around the individual product, such as a polybag, branded box, wrapping, tags or protective inserts.' },
]
...
export function MethodologyPage({ navigate }: { navigate: (page: Page) => void }) {
  ...
  return (
    <div className="methodology-page page-surface">
      <header className="page-hero container method-header"><div><p className="kicker">Published EcoMind methodology</p><h1>One score. Six visible factors.</h1><p>The numeric Green Score is deterministic. AI interprets listing text but never decides the score.</p></div><div className="method-score"><TrafficLightResult score={result.score} hasSufficientEvidence={result.knownWeight >= .35} provisional={result.provisional} confidence={sample.confidenceLevel} grade={result.grade} /><small>sample calculation</small></div></header>
      <div className="container method-content">
        <section className="formula-card"><div><h2>The formula</h2><p>Each factor is normalised from 0 to 100, multiplied by its fixed weight, then rounded to the nearest whole number.</p></div><code>Green Score = M × 0.35 + C × 0.25 + R × 0.20 + D × 0.10 + F × 0.05 + P × 0.05</code></section>
        ...
        <section className="method-caveats"><h2>Important limitations</h2><div><p><strong>Prototype weights.</strong> The six weights are assumptions and have not been scientifically validated.</p><p><strong>Possible overlap.</strong> Material-impact and carbon factors may represent some of the same underlying impacts.</p><p><strong>Missing means unknown.</strong> EcoMind displays “Not disclosed”, reduces confidence and shows a provisional range instead of treating unknown as zero.</p><p><strong>Not a certification.</strong> EcoMind is an educational decision-support tool, not a lifecycle assessment or official label.</p><p><strong>People information stays separate.</strong> Labour ethics is not included in this environmental score.</p><p><strong>Expert review required.</strong> Factors, thresholds, confidence rules and wording require independent review before real-world use.</p></div></section>
        ...
        <section className="future-data"><h2>Packaging has two stages</h2><p>Manufacturer packaging protects the individual product, while fulfilment packaging is added by the retailer or delivery partner. EcoMind scores them separately because one may be sustainable while the other is not.</p><p>A retailer packaging policy may not describe the packaging used for every individual order. Policy-only evidence is labelled “Estimated from retailer policy”, kept separate from product-page evidence and reduces confidence.</p></section>
        <section className="future-data"><h2>Materials and certifications</h2><p>Certifications can strengthen evidence, but their absence does not necessarily mean a product performs poorly. Only relevant, product-specific and independently verifiable environmental certifications can add a small adjustment. Social, labour and chemical-safety evidence stays visibly separate.</p><p>The adjustment is +2 for one independently relevant verified environmental certification and no more than +3 for multiple certifications. Aliases are deduplicated; seller claims and vague marketing language receive zero points; missing certification creates no penalty. Certifications never replace missing carbon or packaging evidence.</p><p><strong>OEKO-TEX® STANDARD 100</strong> shows harmful-substance testing. It is valuable product-safety evidence but does not by itself establish lower lifecycle impact, so it adds zero Green Score points. <strong>OEKO-TEX® MADE IN GREEN</strong> can support environmental and people evidence, but earns the small prototype adjustment only when its unique product ID is confirmed through the official OEKO-TEX Label Check. Seller wording alone adds zero. Fair Trade Certified remains People information and does not increase the environmental score.</p><p>The certification adjustment and +3 cap remain prototype assumptions requiring expert validation.</p></section>
