...
import { ECO_POINT_RULES, addPointEvent, createPointEvent } from '../../shared/ecoPoints'
import { formatTrafficLightScore, getTrafficLightStatus, trafficLightAccessibleText } from '../../shared/trafficLight'
import { queueExtensionEvent, readExtensionState, STORAGE_KEY, writeExtensionState, type ExtensionState, type ExtensionWishlistItem } from './shared'
...
type DisplayAnalysis = { score: number | null; range: [number, number] | null; grade: string | null; confidence: 'High' | 'Medium' | 'Low'; factors: Array<{ key: ScoreFactorKey; label: string; result: FactorResult; weight: number }>; explanation: string; canScore: boolean; provisional: boolean; hasSufficientEvidence: boolean }
interface StatusMessage { type: 'ECOMIND_STATUS_UPDATE' | 'ECOMIND_GET_STATUS'; state?: AnalysisState; detail?: string; score?: number | null; range?: [number, number] | null; grade?: string | null; confidence?: 'High' | 'Medium' | 'Low'; provisional?: boolean; hasSufficientEvidence?: boolean }
...
function notifyPopup(state: AnalysisState, detail: string, analysis?: DisplayAnalysis) {
  analysisState = state; analysisDetail = detail
  chrome.runtime.sendMessage({ type: 'ECOMIND_STATUS_UPDATE', state, detail, ...(analysis ? { score: analysis.score, range: analysis.range, grade: analysis.grade, confidence: analysis.confidence, provisional: analysis.provisional, hasSufficientEvidence: analysis.hasSufficientEvidence } : {}) } satisfies StatusMessage, () => void chrome.runtime.lastError)
}
...
function displayAnalysisForRecord(record: (typeof PRODUCT_RECORDS)[number]): DisplayAnalysis {
  ...
  return { score: result.score, range: result.range ? [result.range.min, result.range.max] : null, grade: result.grade, confidence: record.confidenceLevel, factors, explanation: result.explanation, canScore: true, provisional: result.provisional, hasSufficientEvidence: result.knownWeight >= .35 }
}
...
function displayAnalysis(product: ParsedProduct): DisplayAnalysis {
  ...
  return { score: result.score, range: result.range, grade: result.grade, confidence: result.confidence, factors: (Object.keys(result.factors) as ScoreFactorKey[]).map((key) => ({ key, label: factorLabels[key], result: result.factors[key], weight: SCORE_WEIGHTS[key] })), explanation: result.explanation, canScore: result.canScore, provisional: true, hasSufficientEvidence: result.canScore }
}

function trafficIcon(colour: string) { return colour === 'green' ? '✓' : colour === 'amber' ? '−' : colour === 'red' ? '!' : '?' }
function trafficMarkup(analysis: DisplayAnalysis, compact = false) {
  const status = getTrafficLightStatus(analysis.score, analysis.hasSufficientEvidence)
  const accessible = trafficLightAccessibleText(status, analysis.score, analysis.provisional, analysis.confidence, analysis.range)
...
function renderExtension(product: ParsedProduct, analysis: DisplayAnalysis, state: ExtensionState, diagnostics: ParserDiagnostics) {
  ...
  const trafficStyle = document.createElement('style'); trafficStyle.textContent = trafficStyles; shadow.prepend(trafficStyle)
...
async function recordRealComparison(product: ParsedProduct) {
  ...
  if (!state.completedActions.includes(key)) { const event = createPointEvent({ actionType: 'compareGreenerAlternative', points: ECO_POINT_RULES.compareGreenerAlternative, source: 'extension', deduplicationKey: key, title: 'Real products compared', detail: product.title ?? product.retailer, selfReported: false }); state.pointEvents = addPointEvent(state.pointEvents, event); queueExtensionEvent(state, event); state.completedActions.push(key); state.points += event.points; state.activities.unshift({ id: event.id, title: event.title, detail: event.detail, points: event.points, date: 'Today', timestamp: event.timestamp }); await writeExtensionState(state); showToast(state.backendAccountId ? 'Comparison recorded. Waiting for backend confirmation.' : 'Comparison recorded locally. +5 EcoPoints.') } else showToast('This comparison is already recorded.')
}
...
async function recordThreadlyComparison(product: ParsedProduct) {
  ...
  if (!state.completedActions.includes(key)) { const event = createPointEvent({ actionType: 'compareGreenerAlternative', points: ECO_POINT_RULES.compareGreenerAlternative, source: 'extension', deduplicationKey: key, title: 'Greener demo alternative compared', detail: alternative.productName, selfReported: false }); state.pointEvents = addPointEvent(state.pointEvents, event); queueExtensionEvent(state, event); state.completedActions.push(key); state.points += event.points; state.activities.unshift({ id: event.id, title: event.title, detail: event.detail, points: event.points, date: 'Today', timestamp: event.timestamp }); await writeExtensionState(state); showToast(state.backendAccountId ? 'Comparison waiting for backend confirmation.' : 'Demo comparison recorded locally. +5 EcoPoints.') } else showToast('This demo comparison is already recorded.')
}
...
async function saveThreadlyAlternative(product: ParsedProduct) {
  ...
  if (!state.completedActions.includes(key)) { const event = createPointEvent({ actionType: 'saveLowerImpactOption', points: ECO_POINT_RULES.saveLowerImpactOption, source: 'extension', deduplicationKey: key, title: 'Demo alternative saved', detail: alternative.productName, selfReported: false }); state.pointEvents = addPointEvent(state.pointEvents, event); queueExtensionEvent(state, event); state.completedActions.push(key); state.points += event.points; state.activities.unshift({ id: event.id, title: event.title, detail: event.detail, points: event.points, date: 'Today', timestamp: event.timestamp }) }
...
async function submitManualCorrection(event: SubmitEvent, product: ParsedProduct) {
  ...
  notifyPopup(currentAnalysis.canScore ? (currentAnalysis.confidence === 'Low' ? 'low-confidence' : 'missing-data') : currentProduct.isClothing ? 'missing-data' : 'unsupported-category', 'Manual evidence applied locally. Open EcoMind to review the revised result.', currentAnalysis)
}
...
function recordAnalysis(state: ExtensionState, product: ParsedProduct) {
  ...
  const event = createPointEvent({ actionType: 'analysis', points: 0, source: 'extension', deduplicationKey: key, title: 'Product analysis completed', detail: `${product.retailer}: ${product.title ?? 'manual entry'}`, selfReported: false }); state.pointEvents = addPointEvent(state.pointEvents, event); state.completedActions.push(key); state.activities.unshift({ id: event.id, title: event.title, detail: event.detail, points: 0, date: 'Today', timestamp: event.timestamp })
}
...
async function runAnalysis() {
  try {
    ...
    if (!currentProduct.isProduct) notifyPopup('unsupported', 'EcoMind could not confirm a product page. Open the koala to enter product information manually.', currentAnalysis)
    else if (!currentProduct.isClothing) notifyPopup('unsupported-category', 'Product detected, but EcoMind currently scores clothing and textile products. Manual evidence is available.', currentAnalysis)
    else if (!currentAnalysis.canScore) notifyPopup('missing-data', 'Product detected, but material composition is not disclosed. Manual confirmation is available.', currentAnalysis)
    else if (currentAnalysis.confidence === 'Low') notifyPopup('low-confidence', `${currentProduct.retailer} product analysed with low confidence. Review missing evidence or add a correction.`, currentAnalysis)
    else if (currentAnalysis.factors.some((item) => item.result.status === 'unknown')) notifyPopup('missing-data', `${currentProduct.retailer} product analysed provisionally. Missing factors remain unknown.`, currentAnalysis)
    else notifyPopup('success', `${currentProduct.retailer} clothing product analysed locally. Open the koala for evidence.`, currentAnalysis)
...
chrome.runtime.onMessage.addListener((message: StatusMessage, _sender, sendResponse) => {
  if (message.type === 'ECOMIND_GET_STATUS') sendResponse({ type: 'ECOMIND_STATUS_UPDATE', state: analysisState, detail: analysisDetail, ...(currentAnalysis ? { score: currentAnalysis.score, range: currentAnalysis.range, grade: currentAnalysis.grade, confidence: currentAnalysis.confidence, provisional: currentAnalysis.provisional, hasSufficientEvidence: currentAnalysis.hasSufficientEvidence } : {}) } satisfies StatusMessage)
